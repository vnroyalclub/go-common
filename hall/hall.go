...
// GetGameTaxConfig 获取游戏税收配置
func GetGameTaxConfig(req gproto.GetGameTaxConfig, url string) (rsp gproto.GetGameTaxConfigResp, err error) {
	rsp = gproto.GetGameTaxConfigResp{Taxes: []*gproto.GameTaxConfig{}}
	...
	res, err := httpPost(0, int32(gproto.Operation_OP_S_ReqGameTaxConfig), body, url)
	if err != nil {
		log.Error("failed to close account,err:", err)
		return
	}
	...
	var gameTaxConfigResp gproto.GetGameTaxConfigResp
	err = proto.Unmarshal(res.GetData(), &gameTaxConfigResp)
	if err != nil {
		return
	}

	rsp = gameTaxConfigResp
	return
}

// ReqShipping 请求发货
func ReqShipping(orderID, url string) (isOk bool, err error) {
	var req = gproto.ShippingReq{OrderId: proto.String(orderID)}
	body, err := proto.Marshal(&req)
	if err != nil {
		log.Error("ReqShipping, Marshal err:", err)
		return
	}
	res, err := httpPost(0, int32(gproto.Operation_OP_S_ReqShipping), body, url)
	if err != nil {
		log.Error("ReqShipping, http post err:", err)
		return false, err
	}

	if *res.En != int32(gproto.ErrorCode_Success) {
		log.Error("ReqShipping, errCode:", *res.En)
		err = fmt.Errorf("errNum:%v", *res.En)
		return false, err
	}
